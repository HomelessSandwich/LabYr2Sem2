...
for i in range(1, 7):
    t, y = np.loadtxt("signal{0}.dat".format(i), unpack=True)
    y = y-np.mean(y)
    ...
    print('Maximum value of sample {0} occurs at frequency: {1}Hz'.format(i, str(float(maxValue))))
    ...
    plt.ylabel('abs(FT)')
    plt.savefig('sample{0}.png'.format(i))
    plt.show()
